...
def build_weighted_customers(keys, pct, mult, seed=42):
    """
    Randomly selects % of customers as heavy repeat buyers.
    Heavy buyers appear 'mult' times more frequently.
    """
...
def build_weighted_date_pool(start, end, seed=42):
    ...
    year_index = {y: i for i, y in enumerate(unique_years)}
    growth = 1.08  # +8% YoY
    yw = np.array([growth ** year_index[y] for y in years], float)
    ...
    # ----- Random 10–18% zero-sales blackout days -----
...
def generate_chunk_df(
    ...
    lines_per_order = rng.choice([1,2,3,4,5], order_count,
    ...
    # "hash" trick -> deterministic offset
    ...
    # ============================================================
    # Promotion assignment (FULLY VECTORIZED)
    # ============================================================
    ...
    # ============================================================
    # Price Reduction (COMMENT FIXED — values stay SAME)
    # Correct comment: This reduces prices by ~39–57%.
    # ============================================================

    rng = np.random.default_rng(seed)
    price_factor = rng.uniform(0.43, 0.61)  # correct values — only comment updated
...
def generate_sales_fact(
    ...
    if file_format == "csv":
        merge_parquet = False  # enforce safety for CSV mode
    ...
    # ----- Promotions (pre-vectorized for speed) -----
    ...
    while remaining > 0:
        ...
        if file_format == "csv":
            ...
            write_parquet(df, out, row_group_size=row_group_size,
